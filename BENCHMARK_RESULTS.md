...
- 当前共有一次首次演示和 3 次预注册重复；重复样本仍然很小，1/3 只能描述本次实验，不能外推为通用稳定成功率。
...
公开仓库保存了[原生 smoke 报告](evidence/ai-health-native-smoke-report.md)、[状态污染报告](evidence/ai-health-chat-state-pollution-report.md)、Prompt 演化摘要和三次重复汇总。历史运行前没有记录目标源码修订与 JAR SHA-256，完整审计也没有公开，因此这些材料支持作者完成过真实系统集成，不支持第三方逐位重建当时系统。这个缺口不会用后来的文件身份追溯填补。
