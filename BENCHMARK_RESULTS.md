...
| 证据 | 结果 | 能证明什么 |
| --- | --- | --- |
| 自动化测试 | 38/38 通过 | 规则、演化决策、场景独立门禁、严格 holdout 门禁、多轮继续、冻结数据恢复校验、空集与重复 ID 拒绝、追加式恢复身份、预算、LLM JSON 边界、候选证据隔离、Gold 复核、评测指标、跨进程互斥、SQLite 多写者、多文件回滚、容器安全默认值和进程超时行为符合当前测试口径 |
