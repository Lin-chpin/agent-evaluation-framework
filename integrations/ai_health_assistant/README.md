...
2026-08-31 新增了一次运行前取证的独立重跑。运行前记录了框架 commit 与 clean 状态、健康助手 commit 与 dirty 状态、实际 JAR SHA-256、Java 和模型配置、三套原始数据哈希以及完整命令。公开结果见[可追溯重跑摘要](../../evidence/ai-health-provenance-rerun-summary.json)，接受的沙箱 Prompt 见[公开副本](../../evidence/ai-health-provenance-rerun-accepted-planner.prompt.txt)。

本次生成两个候选。第一个候选虽然提高硬通过率，但让 holdout 增加一个软告警，因此被回滚。第二个候选把 improvement 从 25% 提高到 100%，保持 regression 100%，并把 holdout 从 75% 提高到 100%，最终在沙箱接受。它同时让 holdout 平均延迟增加约 4.13 秒；由于本次没有登记业务延迟门禁，该变化被记录但没有阻断候选。

健康助手工作区在运行前存在 46 条未提交状态，因此 Git commit 不能单独重建源码。JAR SHA-256 固定了实际执行产物。该结果补齐了当前参考运行的执行身份，不把 2026-08-30 的历史运行追溯改写成同一实验。

后续集成实验继续在运行前记录外部项目提交、工作区状态、JAR SHA-256、Java 版本、模型配置和数据集哈希。以下命令展示同一接入流程，不保证随机模型生成相同候选。
