...
| --- | --- | --- |
| Automated tests | 39/39 passed | Current tests cover rules, evolution decisions, independent scenario gates, strict holdout gating, continued multi-round improvement, frozen-dataset resume checks, empty-suite and duplicate-ID rejection, append-only resume identity, budgets, LLM JSON boundaries, candidate-evidence isolation, Gold adjudication, evaluator metrics, cross-process locking, concurrent SQLite writers, multi-file rollback, container security defaults, process timeouts, and output truncation. |
